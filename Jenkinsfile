pipeline {
    ...
    options {
        ...
        buildDiscarder(logRotator(numToKeepStr: '50'))   //保持构建的最大个数
    ...
    parameters {
        ...
        choice( 
          ...
          description: '选择部署环境dev:开发自测环境',
	    
          name: 'ENV',
	    
          choices: ['dev','test', 'uat','rollback','prod']
		...
		string(name: 'APP_ID', defaultValue: '系统简写名称', description: '所属子系统名') //用于匹配namespace，${APP_ID}-${ENV}
    ...
    stages {
        ...
        stage('Environment Init') {
          ...
          when {expression {return params.ENV == 'dev'||params.ENV == 'test'||params.ENV == 'uat'||params.ENV == 'prod'}}
